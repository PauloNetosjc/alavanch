...
BEGIN
  ...
  -- Conta total global (para descrição "X/Y")
  ...
  FOR v_pag IN SELECT * FROM public.pagamentos_orcamento WHERE orcamento_id = v_ped.orcamento_id ORDER BY created_at LOOP
    ...
    -- Forma oficial do método (ex.: "Cartão de Crédito" p/ método "Cartão Crédito 1x a 18x")
    v_metodo_forma := NULL;
    ...
    FOR v_k IN 0..(v_qtd - 1) LOOP
      ...
      -- pertence ao agrupamento do método?
    ...
    -- Lançamento agrupado (se houver parcelas cuja forma bate com o método agrupado)
    IF v_grupo_qtd > 0 THEN
      ...
      IF v_grupo_venc IS NULL THEN
        v_grupo_venc := v_data_base + (v_prazo || ' days')::interval;
      END IF;
