...
ALTER TABLE public.orcamentos ADD COLUMN IF NOT EXISTS is_complemento boolean NOT NULL DEFAULT false;
ALTER TABLE public.orcamentos ADD COLUMN IF NOT EXISTS pedido_origem_complemento_id uuid;

ALTER TABLE public.pedidos ADD COLUMN IF NOT EXISTS is_complemento boolean NOT NULL DEFAULT false;
ALTER TABLE public.pedidos ADD COLUMN IF NOT EXISTS pedido_origem_complemento_id uuid;
...
BEGIN
  IF NEW.status IN ('confirmado','convertido','aprovado')
    ...
    v_prefix := CASE
      WHEN v_is_adendo AND v_pai IS NOT NULL THEN 'AD'
      WHEN v_is_complemento THEN 'COMP'
      ELSE 'PV'
    END;
    ...
    -- Adendo não cria pastas (vive dentro do pai). Complemento e PV criam.
