ALTER TABLE public.solicitacoes_assinatura
  ...
  ADD COLUMN IF NOT EXISTS data_assinatura_manual TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS arquivo_contrato_manual_url TEXT,
  ADD COLUMN IF NOT EXISTS documento_cliente_manual_url TEXT,
  ADD COLUMN IF NOT EXISTS assinado_manualmente_por TEXT,
  ADD COLUMN IF NOT EXISTS assinado_manual_por UUID,
  ADD COLUMN IF NOT EXISTS assinado_manual_em TIMESTAMPTZ;

CREATE OR REPLACE FUNCTION public.registrar_assinatura_manual(p_solic uuid, p_contrato_url text, p_contrato_path text, p_doc_cliente_url text DEFAULT NULL::text, p_doc_cliente_path text DEFAULT NULL::text, p_observacao text DEFAULT NULL::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Atualiza solicitação manual sem apagar histórico, token, evidências ou logs digitais.
  UPDATE public.solicitacoes_assinatura
     SET status = 'assinado_manual'::public.assinatura_status,
         ...
         data_assinatura_manual = now(),
         arquivo_contrato_manual_url = p_contrato_url,
         documento_cliente_manual_url = COALESCE(p_doc_cliente_url, documento_cliente_manual_url),
         assinado_manualmente_por = v_uid::text,
