      ...
      <div className="rounded-2xl border bg-card p-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button variant="secondary"><BarChart3 className="w-4 h-4 mr-1" /> Resultado por Contrato</Button>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-[10px] uppercase tracking-wider text-muted-foreground border-b bg-muted/30">
                ...
                <th className="text-left py-3 font-medium">Descrição / Origem</th>
                <th className="text-right py-3 font-medium">Valor</th>
                <th className="text-left py-3 pl-6 font-medium">Categoria</th>
              ...
              {filtrados.map((l) => {
                ...
                return (
                  <tr key={l.id} className={`border-b hover:bg-muted/30 ${cancelado ? "opacity-60" : ""}`}>
                    <td className="py-4 px-5 whitespace-nowrap">{fmt(l.data_pagamento || l.data_vencimento)}</td>
                    <td>
                      <div className="flex items-center gap-3">
                        <span className={`w-9 h-9 rounded-full flex items-center justify-center ${l.tipo === "entrada" ? "bg-emerald-500/15" : "bg-rose-500/15"}`}>
                          {l.tipo === "entrada"
                    ...
                    <td className={`text-right font-semibold whitespace-nowrap ${l.tipo === "entrada" ? "text-emerald-700" : "text-rose-700"}`}>
                      {BRL(Number(l.valor || 0))}
                    </td>
                    <td className="pl-6">
                      <div>{catName(l.categoria_id)}</div>
                      {catParent(l.categoria_id) && (
                        <div className="text-[11px] text-muted-foreground">→ {catParent(l.categoria_id)}</div>
                      )}
                    </td>
                    <td className="text-center">
                      {cancelado
                        ...
                        : pago
                          ? <Badge className="bg-emerald-500/15 text-emerald-700">LIQUIDADO</Badge>
                          : <Badge className="bg-violet-500/15 text-violet-700">PENDENTE</Badge>}
                    </td>
                    <td className="text-right px-5">
                      <div className="flex justify-end gap-1">
                        {!pago && !cancelado && (
                          <Button size="icon" variant="ghost" title="Liquidar" onClick={() => abrirLiquidar(l)}>
