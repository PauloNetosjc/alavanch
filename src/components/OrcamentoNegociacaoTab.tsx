...
import { Calculator, History, FilePlus2, ArrowRight, Loader2 } from "lucide-react";
...
export default function OrcamentoNegociacaoTab({ orcamentoId }: { orcamentoId: string }) {
  ...
  const novaVersao = async () => {
    ...
    const base: any = { ...ativa };
    delete base.id;
    delete base.created_at;
    delete base.updated_at;
    base.versao = proxVer;
    base.status = "ativa";
    base.criado_por = u.user?.id || null;
    base.observacoes = `Nova versão criada a partir da v${ativa.versao}`;

    const { error: e2 } = await supabase.from("orcamento_negociacoes" as any).insert(base);
    if (e2) return toast.error(e2.message);
    toast.success(`Versão ${proxVer} criada`);
    load();
  };
    ...
    <div className="space-y-5">
      <div className="surface-card p-6">
        <div className="flex items-start justify-between gap-4 flex-wrap mb-5">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-[22px] font-semibold">Negociação atual</h2>
              <Badge variant="outline" className={STATUS_COLOR[ativa.status] || ""}>
                {ativa.status}
              </Badge>
              <Badge variant="outline">v{ativa.versao}</Badge>
            </div>
            <p className="text-[12px] text-muted-foreground mt-1">
              {fmtDate(ativa.created_at)}
              {ativa.criado_por && usuariosMap[ativa.criado_por] && (
                <> · por <b>{usuariosMap[ativa.criado_por]}</b></>
              )}
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2 text-[14px]">
          <Linha label="Valor bruto original" value={fmtBrl(ativa.valor_bruto)} />
          <Linha
            label={`Desconto manual (${(ativa.percentual_desconto_manual || 0).toFixed(2)}%)`}
            value={`- ${fmtBrl(ativa.valor_desconto_manual)}`}
            color="text-rose-700"
          />
          <Linha
            label={`Desconto forma pagamento (${(ativa.percentual_desconto_forma_pagamento || 0).toFixed(2)}%)`}
            value={`- ${fmtBrl(ativa.valor_desconto_forma_pagamento)}`}
            color="text-rose-700"
          />
          <Linha
            label="Valor após desconto da forma pag."
            value={fmtBrl(ativa.valor_apos_desconto_forma_pagamento)}
          />
          <Linha label="Entrada" value={fmtBrl(ativa.valor_entrada)} />
          <Linha
            label={`Desconto da entrada (${(ativa.percentual_desconto_entrada || 0).toFixed(2)}%)`}
            value={`- ${fmtBrl(ativa.valor_desconto_entrada)}`}
            color="text-rose-700"
          />
          <Linha
            label="Valor final negociado"
            value={fmtBrl(ativa.valor_final_negociado)}
            strong
          />
          <Linha label="Saldo a parcelar" value={fmtBrl(ativa.saldo_a_parcelar)} strong />
          <Linha
            label="Parcelamento"
            value={`${ativa.quantidade_parcelas}x de ${fmtBrl(ativa.valor_parcela)}`}
          />
        </div>

        {ativa.observacoes && (
          <div className="mt-5 text-[12px] text-muted-foreground border-t border-border pt-3">
            <b>Observações:</b> {ativa.observacoes}
          </div>
        )}
      </div>
